...
The v1.1 backlog is documented and has rationale attached. Ollama support, domain age via WHOIS, STIX 2.1 export, a SQLite cache for repeat analysis, and the integration story with vex — pipe barb JSON output into vex for combined offline heuristics and VirusTotal enrichment on the same URL. That combination covers two different parts of the analyst workflow with tools that each work independently.
