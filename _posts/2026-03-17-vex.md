...
**Passive version check.** A new `version_check.py` module queries the GitHub releases API, caches the result for 24 hours, and displays a notice when a newer release is available. The MeetUp explicitly decided against an auto-update command — supply chain risk and SOC change management were the reasons. The check fails silently on network errors and never blocks the tool.
...
Four verdict levels: `CLEAN`, `UNKNOWN`, `SUSPICIOUS`, `MALICIOUS`. The important one is `UNKNOWN` — returned when too few engines have scanned a file to say anything reliable. Zero detections is not the same as clean, especially for new or rare samples. This comes up in the [[2026-02-28-invite-only]] writeup too, where VirusTotal community data was more useful than the automated detection count alone.
